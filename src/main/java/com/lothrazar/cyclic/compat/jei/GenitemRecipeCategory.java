...
public class GenitemRecipeCategory implements IRecipeCategory<RecipeGeneratorItem> {
  ...
  public GenitemRecipeCategory(IGuiHelper helper) {
    ...
    icon = helper.drawableBuilder(new ResourceLocation(ModCyclic.MODID, "textures/blocks/machine/fuel_item.png"), 0, 0, 16, 16).setTextureSize(16, 16).build();
  }
  ...
  public void draw(RecipeGeneratorItem recipe, MatrixStack ms, double mouseX, double mouseY) {
    Minecraft.getInstance().fontRenderer.drawString(ms, recipe.getTicks() + " t", 60, 5, FONT);
    Minecraft.getInstance().fontRenderer.drawString(ms, recipe.getRfpertick() + " RF/t", 60, 20, FONT);
  }
