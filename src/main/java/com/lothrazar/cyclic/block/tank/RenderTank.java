...
public class RenderTank extends TileEntityRenderer<TileTank> {
  ...
  public static class LightValue {
    ...
    public LightValue(Integer brightness) {
      l1 = brightness >> 16 & 0xFFFF;
      l2 = brightness >> 0 & 0xFFFF;
    }
  }
  ...
  public void renderTileEntityFast(TileTank tankHere, double x, double y, double z, float partialTicks, int destroyStage, net.minecraft.client.renderer.BufferBuilder bufferDoNotUse) {
    ...
    int minLight = fluidStack.getFluid().isIn(FluidTags.LAVA) ? 1 : 0;
    //
    int brightness = tankHere.getWorld().getCombinedLight(new BlockPos(x, y, z), minLight);
    LightValue lv = new LightValue(brightness);
    lv.l1 = 0;
    lv.l2 = 0;
    //    ModCyclic.LOGGER.info("has " + fluidStack);
    ...
    int icolor = 0xFF000000;//???? fluidStack.getFluid().getColor(fluidStack);
    ...
    buffer.pos(B, posY, single).tex(still.getInterpolatedU(S), still.getInterpolatedV(S)).color(red, green, blue, alph).endVertex();//.lightmap(lv.l1, lv.l2).
    ...
    //    buffer.setTranslation(x, y, z);
    buffer.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX_COLOR);
    buffer.pos(T, posY, B).tex(flow.getMinU(), flow.getMinV()).color(red, green, blue, alph).endVertex();
