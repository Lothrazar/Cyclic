...
public class ContainerUncraft extends ContainerBase {
  ...
  public ContainerUncraft(int windowId, Level world, BlockPos pos, Inventory playerInventory, Player player) {
    ...
    for (int j = 0; j < 2; ++j) {
      for (int k = 0; k < 8; ++k) {
        this.addSlot(new SlotItemHandler(outputSlots, index, 8 + k * Const.SQ, 27 + (j + 1) * Const.SQ) {
  ...
  public boolean stillValid(Player playerIn) {
    return stillValid(ContainerLevelAccess.create(tile.getLevel(), tile.getBlockPos()), playerEntity, BlockRegistry.UNCRAFTER.get());
  }
}
