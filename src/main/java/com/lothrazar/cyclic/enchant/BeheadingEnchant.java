...
public class BeheadingEnchant {
  ...
  public void onEntityKill(LivingDeathEvent event) {
    ...
    int level = EnchantUtil.getCurrentLevelTool(EnchantRegistry.holder(EnchantRegistry.BEHEADING, attacker), attacker);
    if (level <= 0) return;
    Level world = attacker.level();
    if (Mth.nextInt(world.random, 0, 100) > percentForLevel(level)) return;
