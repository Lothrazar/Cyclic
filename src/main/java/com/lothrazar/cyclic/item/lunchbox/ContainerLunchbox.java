...
public class ContainerLunchbox extends ContainerBase {
  ...
  public ContainerLunchbox(int i, Inventory playerInventory, Player player) {
    ...
    this.playerInventory = playerInventory;
      net.neoforged.neoforge.items.IItemHandler h = bag.getCapability(net.neoforged.neoforge.capabilities.Capabilities.ItemHandler.ITEM);
      if (h != null) {
        this.slotCount = h.getSlots();
        this.endInv = h.getSlots();
        for (int j = 0; j < h.getSlots(); j++) {
          this.addSlot(new net.neoforged.neoforge.items.SlotItemHandler(h, j,
              26 + j * Const.SQ,
              36));
        }
      }
