        ...
        if (rightClickDown) {
          if (stackTarget.getItem() instanceof ItemLunchbox
            ...
            if (isEdible) {
              ...
              net.neoforged.neoforge.network.PacketDistributor.sendToServer(new PacketItemGui(slotId, stackTarget.getItem()));
              event.setCanceled(true);
            }
          }
        }
    ...
    try {
      if (rightClickDown && gui.getSlotUnderMouse() != null) {
        ...
        if (maybeCharm.getItem() instanceof IHasClickToggle) {
          net.neoforged.neoforge.network.PacketDistributor.sendToServer(new PacketItemToggle(slotHit.index));
          event.setCanceled(true);
          //            UtilSound.playSound(ModCyclic.proxy.getClientPlayer(), SoundEvents.UI_BUTTON_CLICK);
            ...
            || maybeCharm.getItem() instanceof CraftingBagItem) {
              net.neoforged.neoforge.network.PacketDistributor.sendToServer(new PacketItemGui(slotHit.index, maybeCharm.getItem()));
              event.setCanceled(true);
            }
