...
public class BlockItemCable extends BlockBaseCable implements IHasRecipe {
  ...
  IBlockState getNewState(IBlockAccess world, BlockPos pos) {
    if (!(world.getTileEntity(pos) instanceof TileEntityItemCable))
      return world.getBlockState(pos);
    TileEntityItemCable tile = (TileEntityItemCable) world.getTileEntity(pos);
    BlockPos con = null;
  ...
  public void addCollisionBoxToList(IBlockState state, World worldIn, BlockPos pos, AxisAlignedBB entityBox, List<AxisAlignedBB> collidingBoxes, @Nullable Entity entityIn, boolean p_185477_7_) {
    if (!(worldIn.getTileEntity(pos) instanceof TileEntityItemCable)) {
      return;
    }
    state = state.getActualState(worldIn, pos);
    TileEntityItemCable tile = (TileEntityItemCable) worldIn.getTileEntity(pos);
    float x1 = 0.3125F;
    float x2 = 0.6875F;
    float y1 = 0.3125F;
    float y2 = 0.6875F;
  ...
  public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
    if (source.getTileEntity(pos) instanceof TileEntityItemCable == false) {
      return FULL_BLOCK_AABB;
    }
    state = state.getActualState(source, pos);
    TileEntityItemCable tile = (TileEntityItemCable) source.getTileEntity(pos);
    float x1 = 0.37F;
    float x2 = 0.63F;
    float y1 = 0.37F;
    float y2 = 0.63F;
