...
public class GuiEnderBook  extends GuiScreen
{
	...
	public void initGui()
	{
		...
		buttonNew = new GuiButtonEnderbookNew(buttonIdNew, 
				this.width/2 - w,//x
				20,//y
				w,h,buttonIdNew);
		...
		if(bookStack != null && 
				ItemEnderBook.getLocations(bookStack).size() >= ItemRegistry.maximumSaved)
		{
		...
		for(int i = 0; i < list.size(); i++)
		{
			...
			btn = new GuiButtonEnderbookTeleport(buttonID++, x,y,w,h,buttonText,loc.id);//+" "+loc.id
