...
public class UtilHarvestCrops {
	...
	public static int harvestArea(World world, BlockPos pos, int xRadius, HarestCropsConfig conf) {
		...
		if(dolog)ModMain.logger.info("buildradius = " +xRadius);
		...
		if(dolog)ModMain.logger.info("x = " + xMin+":"+xMax);
		if(dolog)ModMain.logger.info("z = " + zMin+":"+zMax);
		...
		for (int xLoop = xMin; xLoop <= xMax; xLoop++) {
			for (int zLoop = zMin; zLoop <= zMax; zLoop++) {
				...
				if(dolog)ModMain.logger.info("posCurrent = " +posCurrent);
	...
	public static boolean harvestSingle(World world,BlockPos posCurrent, HarestCropsConfig conf){
		...
		if ((blockCheck instanceof BlockStem) && conf.doesHarvestStem) {
			...
			if(dolog)ModMain.logger.info(" BlockMushroom ");
			doBreak = true;
		} else if (blockCheck == Blocks.PUMPKIN && conf.doesPumpkinBlocks) {
			if(dolog)ModMain.logger.info(" pumpkin ");
			doBreak = true;
			doReplant = false;
		} else if (blockCheck == Blocks.MELON_BLOCK && conf.doesMelonBlocks) {
			if(dolog)ModMain.logger.info(" melon_block ");
			doBreak = true;
			doReplant = false;
		}
		...
		if (doBreak) {
			...
			if (doReplant) {// plant new seed
				if(dolog)ModMain.logger.info(" replant ");
