	...
	protected void onImpact(RayTraceResult mop){
		...
		if(mop.entityHit != null){
			...
			if(mop.entityHit instanceof EntityLivingBase){
				...
				// e.addPotionEffect(new PotionEffect(PotionRegistry.frost.id, secondsFrozenOnHit *
				// 20,0));
			}
		}
		...
		if(this.isInWater()){
			...
			if(this.worldObj.getBlockState(posWater) != Blocks.water.getDefaultState()){
				posWater = null;// look for the closest water source, sometimes it was air and we
			...
			}// these other cases do not really fire, i think. unless the entity goes inside a
			...
			else if(this.worldObj.isAirBlock(hit) == false // one below us cannot be air
