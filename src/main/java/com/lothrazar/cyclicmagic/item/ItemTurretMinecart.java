...
public class ItemTurretMinecart extends BaseItem implements IHasRecipe {
  ...
  public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
    ...
    if (!BlockRailBase.isRailBlock(iblockstate)) {
      ...
      if (!worldIn.isRemote) {
        ...
        EntityMinecartTurret entityminecart = new EntityMinecartTurret(worldIn, (double) pos.getX() + 0.5D, (double) pos.getY() + 0.0625D + d0, (double) pos.getZ() + 0.5D);
