...
public class ItemCyclicWand extends Item implements IHasRecipe {
	...
	public boolean shouldCauseReequipAnimation(ItemStack oldStack, ItemStack newStack, boolean slotChanged) {
		...
		if (!slotChanged) { return false;// only item data has changed, so do not
		                                 // animate
		}
	...
	public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		return UtilSpellCaster.tryCastCurrent(worldIn, playerIn, pos, side) ? EnumActionResult.SUCCESS : EnumActionResult.FAIL;
	}
	 ...
	 * 
	 * long timeSinceLast = (world.getTotalWorldTime() - lastUsed) /
	 * Const.TICKS_PER_SEC;
	 * 
	 * if(timeSinceLast < 0){
