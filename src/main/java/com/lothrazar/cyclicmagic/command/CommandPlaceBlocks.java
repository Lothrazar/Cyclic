...
public class CommandPlaceBlocks // extends BaseCommand implements ICommand
{
	 ...
	 * "<line|stair|floor|circle> <dist|radius> [skip] [voffset]";
	 * }
	 * 
	 * @Override
	 * public void execute(MinecraftServer server,ICommandSender sender, String[]
	 * args) throws CommandException
	 * {
	 ...
	 * 
	 * IBlockState placing =
	 * Block.getBlockFromItem(held.getItem()).getStateFromMeta(held.getMetadata())
	 * ;
	 * 
	 * if(args.length == 0)
	 * {
	 ...
	 * try
	 * {
	 * distOrRadius = Integer.parseInt(args[1]);//
	 * Math.min(Integer.parseInt(args[1]), held.stackSize);
	 * }
	 * catch (Exception e)
	 * {
	 * UtilChat.addChatMessage(player, getCommandUsage(sender));
	 ...
	 * if(vertOffset < 0) startPos = startPos.down(vertOffset*-1);//so use puts -2
	 * means go down 2
	 * 
	 * if(type.equalsIgnoreCase("line"))
	 * {
	 * //UtilPlaceBlocks.line(player.worldObj, player, startPos, placing,
	 * distOrRadius, skip);//,vertOffset
	 * }
	 * if(type.equalsIgnoreCase("stair"))
	 * {
	 * UtilPlaceBlocks.stairway(player.worldObj, player, startPos, placing,
	 * distOrRadius);//, skip,vertOffset
	 * }
	 * if(type.equalsIgnoreCase("floor"))
	 * {
	 * UtilPlaceBlocks.square(player.worldObj, player, startPos, placing,
	 * distOrRadius);//, skip,vertOffset
	 * }
	 * if(type.equalsIgnoreCase("circle"))
	 * {
	 * UtilPlaceBlocks.circle(player.worldObj, player, startPos, placing,
	 * distOrRadius);//, skip,vertOffset
	 * }
	 * }
	 * 
	 ...
	 * EntityPlayer player = (EntityPlayer)sender;
	 * 
	 * if(player == null){return false;}//was sent by command block or something,
	 * ignore it
	 * 
	 ...
	 * // PlaceLib.allowed =
	 * ModCommands.getBlockListFromCSV(PlaceLib.allowedFromConfig);
	 * 
	 * }
	 * 
