...
public class F3InfoModule extends BaseEventModule implements IHasConfig {//TODO: no f3? eh?  
  ...
  public void addSpawnInfo(RenderGameOverlayEvent.Text event) {
    ...
    if (spawnDistanceEnabled) {
       ...
       * 
       * The exact rule includes chunks whose center is less than or equal to
       * 128 blocks away from the world spawn along both axes. In the rare case
       * where the world spawn is located at the exact center of a chunk, 17
       * chunks will be loaded along that axis, of which 13 activate entities.
       */
